...
2. **채점은 서버에서만**. 클라의 실행 결과로 포인트 주면 조작 가능 → `submitSolution`이
   `problemTests`(운영자만 read)를 읽어 Piston으로 직접 실행/비교하고 지급한다.
...
## 컴파일러 (채점 엔진)
- **Piston 공개 API**(`https://emkc.org/api/v2/piston`) — 키 불필요, C#(mono), stdin 지원.
  공개 인스턴스는 초당 ~5건 rate limit → `submitSolution`은 케이스를 순차 실행(사이 250ms).
- 바꾸려면 `functions/piston.js`의 `PISTON_URL`만 교체(자체 호스팅/Judge0 래핑 등).

## 코드 지도
`functions/`: index.js(runCode·submitSolution·upsertProblem·deleteProblem), piston.js(실행+출력정규화).
